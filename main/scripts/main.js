 ...
 */
"use strict";
var gtris_version = '1.1.5474 Online Test Alpha, Release O_02QF2';
var syncTime = 0;
var StartTime = 0
var syncFrame = 0;
var actualFrame = 0;
...
class DateSynchronizedLoopHandler {
	...
	#asyncRun() {
		...
		if (this.confirmIsAsync !== this.isAsynchronous) {
			...
			if (!this.isAsynchronous) this.startSyncLoop(true);
		}
	...
	startSyncLoop(t) {
		...
		this.interval = setInterval(() => {
			let syncTime = Math.floor((Date.now() - this.startTime)) / (1000 / this.fps);
...
const dslhGame = new DateSynchronizedLoopHandler(60, () => {
	G_LOOP();
});
dslhGame.start();
const glpsounds = {
	alarm: new NumberChangeFuncExec(null, (num, args) => {
		if (num) {
	...
	tpw: new NumberChangeFuncExec(null, (num, args) => {
		if (num) {
...
const _canvasses = {
	...
	frenzy: docId('frenzyCanvas'),
	frenzy2: docId('frenzyCanvas2')
}
const _CTX = {
	...
	frenzy: _canvasses.frenzy.getContext('2d'),
	frenzy2: _canvasses.frenzy2.getContext('2d')
	
}

const class_game = new function() {
	this.touchesPressed = 0;
	this.keysPressed = 0;
	this.BROWSER_FONT_SIZE = parseFloat(window.getComputedStyle(document.documentElement).fontSize.split("px")[0]);
	this.DEFAULT_FONT_SIZE = 16;
	this.FONT_SIZE_PER_PX = this.DEFAULT_FONT_SIZE / this.BROWSER_FONT_SIZE;
	this.resize = function(width, height) {
		...
		/* gachatrisCore.CELL_SIZE = Math.round(aspectRatioResolution / 50);
		 gachatrisCore.PLAYERCLASSAREA_SIZE = gachatrisCore.CELL_SIZE * 0.85;
		 let sizeMult = 1,
		  arrangeInt;
...
function RESIZE() {
	...
	var frenzyCanvas = docId('frenzyCanvas');
	...
	var frenzyCanvas2 = docId('frenzyCanvas2');
	...
	if (varSize === 1 && screenHeight > 102) _cellSize = 12;
	else if (varSize === 2 && screenHeight > 302) _cellSize = 17;
	else if (varSize === 3 && screenHeight > 502) _cellSize = 27;
	else if (varSize === 4 && screenHeight > 702) _cellSize = 39;
	else _cellSize = Math.max(~~(screenHeight / 30), 7);
	cellSize = _cellSize;
	gameFontSize = _cellSize * class_game.FONT_SIZE_PER_PX;
	
	var padNum = (SCREEN_HEIGHT - (cellSize * 20 * tetrionSize + 2)) / 2.3;
	var padNum2 = (SCREEN_HEIGHT - (cellSize * 20 * tetrionSize + 20)) / 1.5
	var padNum3 = (SCREEN_HEIGHT - (cellSize * 20 * tetrionSize)) / 15
	var padNum4 = (SCREEN_HEIGHT - (cellSize * 20 * tetrionSize + 2)) / 2.7;
	
	var pad = padNum + 'px'
	
	class_game.paddingX = getElemPos("core", "x");
	class_game.paddingY = getElemPos("core", "y");
	
	class_game.resize();
	
	mobileButtons.checkButtons();
	
	core.style.width = `${SCREEN_WIDTH}px`;
	core.style.height = `${SCREEN_HEIGHT}px`;
	...
	tetrionSize = 1;
	totalTetrionSize = cellSize * tetrionSize;
	...
	for (let g of $tag('gtris-text-large')) {
		g.style.fontSize = `${gameFontSize*1.8}px`
	}
	
	for (let g of gTrisButtons) {
		g.style.height = `${gameFontSize*0.9}px`
		g.style.width = `${cellSize*10.5}px`
		g.style.fontSize = `${gameFontSize*0.9}px`
	}
	
	logoHeader.title.style.fontSize = `${gameFontSize}px`
	
	logoSplash.style.width = logoSplash.style.height = `${cellSize*16}px`
	
	docId('tSpin').style.fontSize = `${gameFontSize*0.75*tetrionSize}px`
	docId('regular').style.fontSize = `${gameFontSize*1.2*tetrionSize}px`
	docId('B2B').style.fontSize = `${gameFontSize*0.5*tetrionSize}px`
	docId('tSpin').style.height = `${gameFontSize*0.75*tetrionSize}px`
	docId('regular').style.height = `${gameFontSize*1.2*tetrionSize}px`
	docId('B2B').style.height = `${gameFontSize*0.5*tetrionSize}px`
	docId('REN').style.height = `${gameFontSize*1.3*tetrionSize}px`
	docId('REN').style.fontSize = `${gameFontSize*1.3*tetrionSize}px`
	
	/****/
	docId('tetrionPC').style.padding = `${padNum + (cellSize*tetrionSize*6)}px 0`
	
	//docId('tetrion').style.height = docId('tetrion2').style.height = (totalTetrionSize * 20)
	contentLayer.style.padding = `${padNum4}px 0`
	for (var e of resultText)
		e.style.fontSize = `${gameFontSize*3*tetrionSize}px`
	resultImg.style.height = `${cellSize*23*tetrionSize}px`
	resultImg.style.width = `${cellSize*23*tetrionSize}px`
	
	leftBorder.style.width = `${cellSize*5*tetrionSize}px`
	rightBorder.style.width = `${cellSize*5*tetrionSize}px`
	...
	holdPlaceholder.style.height = `${cellSize*1*tetrionSize}px`
	_canvasses.hold.height = cellSize * 3
	_canvasses.hold.style.height = `${_canvasses.hold.height*tetrionSize}px`
	_canvasses.hold.width = cellSize * 5
	_canvasses.hold.style.width = `${_canvasses.hold.width*tetrionSize}px`
	
	holdDiv.style.height = `${cellSize*5*tetrionSize}px`
	
	nextPlaceholder.style.height = `${cellSize*1*tetrionSize}px`
	_canvasses.next.height = cellSize * 3
	_canvasses.next.style.height = `${_canvasses.next.height*tetrionSize}px`
	_canvasses.next.width = cellSize * 5
	_canvasses.next.style.width = `${_canvasses.next.width*tetrionSize}px`
	
	
	_canvasses.queue.height = cellSize * 3 * 10
	_canvasses.queue.style.height = `${_canvasses.queue.height*0.5*tetrionSize}px`
	_canvasses.queue.width = cellSize * 6
	_canvasses.queue.style.width = `${_canvasses.queue.width*0.5*tetrionSize}px`
	
	
	_canvasses.field.height = cellSize * 20.4
	_canvasses.field.style.height = docId('characterBackground').style.height = `${_canvasses.field.height*tetrionSize}px`
	_canvasses.field.width = cellSize * 10
	_canvasses.field.style.width = docId('characterBackground').style.width = `${_canvasses.field.width*tetrionSize}px`
	
	_canvasses.active.height = cellSize * 20.4
	_canvasses.active.style.height = `${_canvasses.active.height*tetrionSize}px`
	_canvasses.active.width = cellSize * 10
	_canvasses.active.style.width = `${_canvasses.active.width*tetrionSize}px`
	
	_canvasses.grid.height = cellSize * 20.4
	_canvasses.grid.style.height = `${_canvasses.grid.height*tetrionSize}px`
	_canvasses.grid.width = cellSize * 10
	_canvasses.grid.style.width = `${_canvasses.grid.width*tetrionSize}px`
	
	
	
	playField.style.height = frenzyField.style.height = frenzyColor.style.height = frenzyCanvas2.style.height = `${_canvasses.field.height*tetrionSize}px`
	playField.style.width = frenzyField.style.width = frenzyColor.style.width = frenzyCanvas2.style.width = `${_canvasses.field.width*tetrionSize}px`
	//dynamicFrenzy.style.width = dynamicFrenzy.style.height = `${cellSize * 26*tetrionSize}px`
	
	for (var e in meterBar) {
		meterBar[e].style.height = `${playField.style.height}`
		meterBar[e].style.width = `${cellSize*0.6*tetrionSize}px`
	}
	
	namePlateDiv.style.height = `${cellSize * 1.1*tetrionSize}px`
	namePlateDiv.style.width = `${cellSize * 13.5*tetrionSize}px`
	namePlateDiv.style.top = `${cellSize * 21.4*tetrionSize}px`
	
	var rectAnim = docId("rectAnimDiv")
	_canvasses.rectAnim.width = cellSize * 11
	_canvasses.rectAnim.height = cellSize * 3
	rectAnim.style.width = `${_canvasses.rectAnim.width * 1.1*tetrionSize}px`
	rectAnim.style.height = `${_canvasses.rectAnim.height * 1.1*tetrionSize}px`
	rectAnim.style.top = `${padNum + (_canvasses.field.height*tetrionSize) + (totalTetrionSize * 1.0) }px`
	
	clearText.style.height = `${cellSize*3*tetrionSize}px`
	statistics.style.width = `${_canvasses.field.width*tetrionSize}px`
	statistics.style.height = leftBorder.style.height
	statistics.style.bottom = 0
	statistics.style.left = `${(-cellSize * 5.1 * tetrionSize)}px`
	
	{
		docId('tetrionPC2').style.padding = `${padNum + (cellSize*6*tetrionSize)}px 0`
		
		contentLayer2.style.padding = `${padNum4}px 0`
		
		resultImg2.style.height = `${cellSize*23*tetrionSize}px`
		resultImg2.style.width = `${cellSize*23*tetrionSize}px`
		
		leftBorder2.style.width = `${cellSize*5*tetrionSize}px`
		rightBorder2.style.width = `${cellSize*5*tetrionSize}px`
		...
		holdPlaceholder2.style.height = `${cellSize*1*tetrionSize}px`
		_canvasses.hold2.height = cellSize * 3
		_canvasses.hold2.style.height = `${_canvasses.hold2.height*tetrionSize}px`
		_canvasses.hold2.width = cellSize * 5
		_canvasses.hold2.style.width = `${_canvasses.hold2.width*tetrionSize}px`
		
		holdDiv2.style.height = `${cellSize*5*tetrionSize}px`
		
		nextPlaceholder2.style.height = `${cellSize*1*tetrionSize}px`
		_canvasses.next2.height = cellSize * 3
		_canvasses.next2.style.height = `${_canvasses.next2.height*tetrionSize}px`
		_canvasses.next2.width = cellSize * 5
		_canvasses.next2.style.width = `${_canvasses.next2.width*tetrionSize}px`
		
		
		_canvasses.queue2.height = cellSize * 3 * 10
		_canvasses.queue2.style.height = `${_canvasses.queue2.height*tetrionSize*0.5}px`
		_canvasses.queue2.width = cellSize * 6
		_canvasses.queue2.style.width = `${_canvasses.queue2.width*tetrionSize*0.5}px`
		
		
		_canvasses.field2.height = cellSize * 20.4
		_canvasses.field2.style.height = docId('characterBackground2').style.height = `${_canvasses.field2.height*tetrionSize}px`
		_canvasses.field2.width = cellSize * 10
		_canvasses.field2.style.width = docId('characterBackground2').style.width = `${_canvasses.field2.width*tetrionSize}px`
		
		_canvasses.active2.height = cellSize * 20.4
		_canvasses.active2.style.height = `${_canvasses.active2.height*tetrionSize}px`
		_canvasses.active2.width = cellSize * 10
		_canvasses.active2.style.width = `${_canvasses.active2.width*tetrionSize}px`
		
		_canvasses.grid2.height = cellSize * 20.4
		_canvasses.grid2.style.height = `${_canvasses.grid2.height*tetrionSize}px`
		_canvasses.grid2.width = cellSize * 10
		_canvasses.grid2.style.width = `${_canvasses.grid2.width*tetrionSize}px`
		
		playField2.style.height = frenzyField2.style.height = frenzyColor2.style.height = frenzyCanvas2.style.height = `${_canvasses.field2.height*tetrionSize}px`
		playField2.style.width = frenzyField2.style.width = frenzyColor2.style.width = frenzyCanvas2.style.width = `${_canvasses.field2.width*tetrionSize}px`
		//dynamicFrenzy2.style.width = dynamicFrenzy2.style.height = `${cellSize * 26*tetrionSize}px`
		
		
		namePlateDiv2.style.height = `${cellSize * 1.1*tetrionSize}px`
		namePlateDiv2.style.width = `${cellSize * 13.5*tetrionSize}px`
		namePlateDiv2.style.top = `${cellSize * 21.4*tetrionSize}px`
		
		var rectAnim2 = docId("rectAnimDiv2")
		_canvasses.rectAnim2.width = cellSize * 11
		_canvasses.rectAnim2.height = cellSize * 3
		rectAnim2.style.width = `${_canvasses.rectAnim2.width * 1.1*tetrionSize}px`
		rectAnim2.style.height = `${_canvasses.rectAnim2.height * 1.1*tetrionSize}px`
		rectAnim2.style.top = `${padNum + (_canvasses.field.height*tetrionSize) + (totalTetrionSize * 1.0) }px`
		
		clearText2.style.height = `${cellSize*3*tetrionSize}px`
		statistics2.style.width = `${_canvasses.field2.width*tetrionSize}px`
		statistics2.style.height = leftBorder2.style.height
		statistics2.style.bottom = 0
		statistics2.style.left = `${-cellSize * 5.1 * tetrionSize}px`
		docId('tSpin2').style.fontSize = `${gameFontSize*0.75*tetrionSize}px`
		docId('regular2').style.fontSize = `${gameFontSize*1.25*tetrionSize}px`
		docId('B2B2').style.fontSize = `${gameFontSize*0.5*tetrionSize}px`
		docId('tSpin2').style.height = `${gameFontSize*0.75*tetrionSize}px`
		docId('regular2').style.height = `${gameFontSize*1.25*tetrionSize}px`
		docId('B2B2').style.height = `${gameFontSize*0.5*tetrionSize}px`
		docId('REN2').style.height = `${gameFontSize*1.3*tetrionSize}px`
		docId('REN2').style.fontSize = `${gameFontSize*1.3*tetrionSize}px`
		
	}
	
	_canvasses.frenzy.width = _canvasses.frenzy2.width = cellSize * 10;
	_canvasses.frenzy.height = _canvasses.frenzy2.height = cellSize * 20.4;
	
	playerIterate(player => {
		player.frenzyBg.cellSize = cellSize;
		/*docId(player.field.assets.frenzy).width = cellSize * 10;
		docId(player.field.assets.frenzy).height = cellSize * 20.4;*/
	});
	docId('tetrion').style.width = docId('tetrion2').style.width = (totalTetrionSize * 25) + "px";
	...
	document.documentElement.style.fontSize = `${gameFontSize*0.9}px`
	for (var i of header1) {
		i.style.fontSize = `${gameFontSize*2}px`
	}
...
function drawSkin() {
	...
	var _default = settingsList.Other.Skin[selectedSettings.Other.Skin].toLowerCase();
	clear(_CTX.sprite)
	clear(_CTX.spriteParticle)
	spriteCustom.src = `skin/${_default}/block.png`
	spriteCustom.onload = () =>
		_CTX.sprite.drawImage(spriteCustom, 0, 0, spriteCustom.width, spriteCustom.height)
	spriteCustomParticle.src = `skin/${_default}/particle.png`
...
const gotoDiscord = () => {
	...
	a.href = "https://discord.com/invite/mnSQJQEuXy"
	a.target = "#";
	a.click();
};
...
var settingsRange = {
	Tuning: {
		...
		GRAV: (function() {
			...
			arr.push('Normal')
			arr.push('0G')
			for (let fraction = 1; fraction < 60; fraction++)
				arr.push(`${fraction}/60G`);
...
var settingsList = {
	Sound: {
		SoundBank: ['Default', "Default 2", 'JavaScriptus', 'Tetra Legends', 'TETR.IO'],
		Music: ['Menu', 'Default Rock', 'The Power']
	},
	NonIterable: {
		Character: ['No Character', 'EricLenovo', 'Betelgeuse Abbygaile', 'Sun Gabbryielle', 'Pikumon10', 'Forest', 'Mars', 'Flotalendy', 'ElishDimensions', 'Evelyn', 'Gahmff', 'RubyStudios', 'Mia', 'Dominic Zi', 'Epicman33456', "Lunar Estelle", "Misty Firefox", "Its Aina Gacha", "EdgeBlackSword", "RedBot", "Nyan Yoshi", "NightstarZi", "Peanutz", "Rainuff"],
	},
	Other: {
...
function pause() {
	docId("guiButton-p-retry").style.display = !server.isOnline ? "block" : "none";
	if ((!server.isOnline || isReplay) && gameRunning && !isPaused && GtrisLoader.loaded) {
		isPaused = true
...
function unPause() {
	if ((!server.isOnline || isReplay) && isPaused && gameRunning && GtrisLoader.loaded) {
		isPaused = false
...
function initialize(mode, parameter, isOnline, oparam) {
	if (mode == 'REPLAY') {
		isReplay = true
		...
		replayData.name = selectedSettings.Names.Main
		actualCustomInit(mode, isOnline, oparam)
		gameMode = mode
	}
	//let udjdj = replayData.seed;
	gachamino.rng.seed = field.rng.seed = field.garbagerng.seed = replayData.seed;
	if ("player2" in replayData) {
		gachamino2.rng.seed = field2.rng.seed = field2.garbagerng.seed = replayData.player2.seed;
	}
	...
	gtrisAI.aiMode = ["tetraplus", "sapphirus"][selectedSettings.AI.AI]
	...
	if ("player2" in replayData) {
		...
		for (let i in replayData.player2.tuning) {
	...
	gachamino.held = false
	gachamino2.held = false
	customInit()
	...
	playerIterate(player => {
		player.field.checkWarning("reset");
	})
...
const initializeLoop = function() {
	...
	if (!gameRunning) {
		/*gameRunner = setInterval(function() {
		 syncTime = Math.floor((Date.now() - StartTime) / (1000 / (!gameRunning || true ? MAIN_FPS : 1)))
		 syncFrame = syncTime - actualFrame
		 for (var i = 0; i < syncFrame; i++, actualFrame++) {
		  G_LOOP()
		  while (gameRunning && gachamino.y < 1 && false) {
		   G_LOOP();
		  }
		 }
		 //while(gachamino.y < 1 && stoppingFrame == 400 || Math.min(stoppingFrame !== 400 ? 1 : 0,1) == 1) G_LOOP();
		 
		}, 1000 / MAIN_FPS)*/
		dslhGame.confirmIsAsync = false;
	}
	
	gameRunning = true
}

function gameStart(gametype, parameter, isOnline, oparam) {
	if (GtrisLoader.loaded) {
		...
		initialize(gametype, parameter, isOnline, oparam);
		activeMenu(false)
		docId('gtris').style.display = 'flex'
		mobileButtons.showHide(true)
		musicPlayer.muteAllMfx(false)
		mobileButtons.enableButtons(true)
		mobileButtons.enableControllers(true)
	}
}
...
function actualCustomInit(mode, isOnline, oparam) {
	switch (mode) {
		case 1: {
			replayData.lineRun = {
		...
		case 10: {
			...
			if (isOnline) {
				replayData.tuning = {
					DAS: oparam.player1.das,
					ARR: oparam.player1.arr,
					GRAV: 0,
					SFT: oparam.player1.sft,
					LCK: 30,
					PREV: 5,
				};
				replayData.seed = oparam.player1.seed;
				replayData.name = oparam.player1.name;
				replayData.character = oparam.player1.character;
				replayData.player2.tuning = {
					DAS: oparam.player2.das,
					ARR: oparam.player2.arr,
					GRAV: 0,
					SFT: oparam.player2.sft,
					LCK: 30,
					PREV: 5,
				};
				replayData.player2.seed = oparam.player2.seed;
				replayData.player2.name = oparam.player2.name;
				replayData.player2.character = oparam.player2.character;
				//console.log(oparam.player2.seed, oparam.player1.seed)
			}
...
function customInit() {
	var mode = gameMode
	playerIterate(player => {
		player.frenzyBg.moveEye(5, -2)
	})
	$iH('namePlate', replayData.name == '' ? `???${selectedSettings.NonIterable.Character !== 0 ? ` (${gtris_character_details(settingsList.NonIterable.Character[selectedSettings.NonIterable.Character]).name})` : ''}` : replayData.name)
	...
	field.isPowerMode = false;
	field2.isPowerMode = false;
	for (var i = 2; i < 4; i++) {
		s2(i, ' ')
		t2(i, ' ')
	}
	...
	switch (mode) {
		...
		case 7: {
			...
			switch (replayData.levelrun.aretype) {
				...
				case 1: {
					field.are.add.line = 10
					field.are.add.piece = 8
					break
				}
				case 2: {
					field.are.add.line = 30
					field.are.add.piece = 30
					break
				}
			}
...
function G_LOOP() {
	...
	if (gameRunning && !isPaused) {
		...
		if (isReplay && (frame in replayData.garbage)) {
			if (replayData.garbage[frame].length > 0)
				for (let g of replayData.garbage[frame]) field.addGarbageToArray(g);
		}
		if ("player2" in replayData) {
			if (isReplay && (frame in replayData.player2.garbage)) {
				if (replayData.player2.garbage[frame].length > 0)
					for (let g of replayData.player2.garbage[frame]) field2.addGarbageToArray(g);
			}
			if (frame == MAIN_FPS * 3 && field2.are.frenzyEnt <= 0) {
				gachamino2.new(preview2.next())
			}
		}
		if (frame == MAIN_FPS * 3 && field.are.frenzyEnt <= 0) {
		...
		if ("player2" in replayData)
			if (field2.isActive)
				P_UPDATE2(isReplay ? 1 : !server.isOnline)
		if (frame > MAIN_FPS * 3) {
			playingFrame++
			if (field.isActive)
				P_ARE()
		...
		if (field.warning || field2.warning) isWarning = 1;
		if (gachamino.lockoutActive || gachamino.lockoutActive) isTopOut = 1;
	} else if (!gameRunning) {
	...
	glpsounds.alarm.assign(isWarning);
	glpsounds.tpw.assign(isTopOut);
	
	if (!isReplay && server.isOnline && server.isRunning) {
		//let g = new TextEncoder.encode(name);
		if (soundPoolOnline.length > 0) {
			let namelength = 0;
			for (let h of soundPoolOnline) {
				namelength += 1 + h.length;
			}
			let wr = server.createWriter("PLAYER_SOUND", namelength + 1);
			wr.u8(soundPoolOnline.length);
			for (let h of soundPoolOnline) {
				wr.string(h, 8);
			}
			server.send(wr.buffer);
			soundPoolOnline.length = 0;
		}
		
	}
	
}
const soundPoolOnline = [];

function P_UPDATE() {
	if (!isReplay) keysPressed = class_game.keysPressed | class_game.touchesPressed;
	...
	if (!isReplay && server.isOnline && server.isRunning && frame % 5 === 0) {
		let wr = server.createWriter("PLAYER_MATCH_DATA", 1 + 2 + 2 + 210 + 3 + 5 + 6 + 4);
		
		let sg = gachamino.index !== 'reset' ? gachamino.index : 0;
		wr.u8(gachamino.index !== 'reset' ? 1 : 0); // active piece show, 1byte
		let ml = pieces[sg];
		let plen = ml.size;
		//wr.u8(plen);
		wr.u8(sg);
		...
		wr.i8(gachamino.x); //1byte
		wr.i8(Math.max(-48, gachamino.y)); //1byte
		for (let x = 0; x < field.width; x++) {
			for (let y = field.height - 21; y < field.height; y++) { // 210 Uint8
				wr.u8(field.grid[x][y] ? field.grid[x][y] : 0);
			}
		}
		wr.u8(gachamino.held ? 1 : 0); //1byte
		wr.i8(hold.piece !== void 0 ? hold.piece : -1); //1byte
		wr.u8(replayData.tuning.PREV); //1byte
		for (let h = 0; h < replayData.tuning.PREV; h++) {
			wr.u8(preview.grabBag[h]); //5byte
		}
		wr.u8(Math.min(21, field.garbageArray.length)); //1byte
		
		wr.u8(field.warning ? 1 : 0); //1byte
		wr.u16(field.pieces); //2byte
		wr.u32(field.score); //2byte
		wr.u16(field.statistics.atk); //2byte
...
function P_ARE() {
	if (field.are.line >= 0 || field.are.piece >= 0 || field.are.del >= 0 || field.are.next >= 0 || field.are.frenzyEnt >= 0 || field.are.frenzyExt >= 0) {
		...
		if (field.are.line == 0) {
			field.removeLines()
			if (field.are.piece < 0) {
				if (frame > MAIN_FPS * 3 && (field.isFrenzy || field.is1v1 !== "frenzywar"))
					gachamino.new(preview.next())
			}
		}
		if (field.are.frenzyEnt == 0) {
			field.are.add.piece = 5
			field.are.add.line = 25
			field.are.del = 15
		...
		if (field.are.del == 0 && !field.isFrenzy && !field.frenzy.endingFrenzy && field.frenzy.startingFrenzy) {
			...
			field.renInteger = -1
			field.b2b = -1
			PLAYERS[0].frenzyBg.moveEye(5, 10, true);
			PLAYERS[0].frenzyBg.isOn = true;
			gachamino.openHold()
			field.are.del = 50
	...
	if (PLAYERS[0].frenzyBg.isOn) PLAYERS[0].frenzyBg.run();
}
...
function P_ARE2() {
	if (field2.are.line >= 0 || field2.are.piece >= 0 || field2.are.del >= 0 || field2.are.next >= 0 || field2.are.frenzyEnt >= 0 || field2.are.frenzyExt >= 0) {
		...
		if (field2.are.line == 0) {
			field2.removeLines()
			if (field2.are.piece < 0) {
				if (frame > MAIN_FPS * 3(field2.isFrenzy || field2.is1v1 !== "frenzywar"))
					gachamino2.new(preview2.next())
			}
		}
		if (field2.are.frenzyEnt == 0) {
			field2.are.add.piece = 5
			field2.are.add.line = 25
			field2.are.del = 15
		...
		if (field2.are.del == 0 && !field2.isFrenzy && !field2.frenzy.endingFrenzy && field2.frenzy.startingFrenzy) {
			...
			field2.renInteger = -1
			field2.b2b = -1
			PLAYERS[1].frenzyBg.isOn = true;
			PLAYERS[1].frenzyBg.moveEye(5, 10, true);
			gachamino2.openHold()
			field2.are.del = 50
	...
	if (PLAYERS[1].frenzyBg.isOn) PLAYERS[1].frenzyBg.run();
}

function Statistics() {
	if (gameRunning && frame > MAIN_FPS * 3) {
		var time = playingFrame
		var seconds = ((time / MAIN_FPS) % 60).toFixed(3)
...
function mforceEnd() {
	...
	server.isPrepared = false;
	server.isRunning = false;
	stoppingFrame = -60 * 3;
	field.checkWarning('stop')
	if ("player2" in replayData)
		field2.checkWarning('stop')
	stopFrame = frame - 1
	isPaused = false
	gameRunning = false
	countDownText('')
	titleTemplate = gtris_transText("forceended");
...
function endGame(title, winlose) {
	...
	if (server.isOnline) {
		let lenk = replayData.keyList;
		let leng = replayData.garbage;
		let lm = JSON.stringify({
			keys: lenk,
			garbage: leng
		});
		let wr = server.createWriter("PLAYER_SUBMIT_REPLAYDATA", (new TextEncoder().encode(lm).byteLength + 32));
		wr.string(lm, 32);
		server.send(wr.buffer);
	}
...
function showEndMenu() {
	...
	dslhGame.confirmIsAsync = true;
	activeMenu(true, 0.5, true)
	switchMenu((server.isOnline) ? 16 : 8, true, typeof title !== 'object' ? gtris_transText(title) : gtris_transText(title.name, title.array), "startPoint")
...
function placeStats(array) {
	...
	for (let D of array) {
		switch (D.toLowerCase()) {
			...
			case 'pieces': {
				str = gtris_transText('ss_pieces', [field.pieces, (field.pieces / ((stopFrame - (MAIN_FPS * 3)) / MAIN_FPS)).toFixed(3)])
				break
			}
			case 'level': {
...
function G_ACTIVITY() {
	switch (gameMode) {
		default: {
			...
			$iH('TEXT_stats1', gtris_transText('pieces', (field.pieces / ((frame - (MAIN_FPS * 3)) / MAIN_FPS)).toFixed(3)))
		...
		case 6: {
			...
			if (!isReplay) garbageSurvival.run(playingFrame, function(count, row) {
				field.addGarbageToArray(count, row)
			})
			$iH("stats2", field.statistics.atk)
			$iH("TEXT_stats2", gtris_transText("survival_apm", (field.statistics.atk / (playingFrame / (MAIN_FPS * 60))).toFixed(3)))
			break
		}
		case 7: {
			$iH('stats1', `${field.pieces}`)
		...
		case 9: {
			...
			amogusSus.run(frame > MAIN_FPS * 3)
			amogusSus.detect(
				field.pieces,
				field.grid,
				function(e) {
		...
		case 10: {
			...
			$iH('TEXT_stats2-1', gtris_transText('pieces', (field2.pieces / ((frame - (MAIN_FPS * 3)) / MAIN_FPS)).toFixed(3)))
			$iH('stats2', `${field.statistics.atk}`)
			$iH('TEXT_stats2', gtris_transText('onevonegarb_apm', (field.statistics.atk / (playingFrame / (MAIN_FPS * 60))).toFixed(3)))
			$iH('stats2-2', `${field2.statistics.atk}`)
			$iH('TEXT_stats2-2', gtris_transText('onevonegarb_apm', (field2.statistics.atk / (playingFrame / (MAIN_FPS * 60))).toFixed(3)))
			break
		}
		case 11: {
				...
				function(time) {
					switch (time) {
						...
						case 13 * MAIN_FPS: {
							...
							playerIterate(player => {
								player.frenzyBg.isOn = true;
								player.frenzyBg.moveEye(5, 10);
							});
							...
							field.openFrenzy(true)
							field2.openFrenzy(true)
							//docId(field.mainAssets.dynamicFrenzyBg).offsetHeight;
							//docId(field2.mainAssets.dynamicFrenzyBg).offsetHeight;
							
							docId(field.mainAssets.colorFrenzyOverlay).style.animation = "frenzyWar-rainbow 0.4s infinite linear"
							//docId(field.mainAssets.dynamicFrenzyBg).style.animationDuration = `0.2s`
							...
							//docId(field2.mainAssets.dynamicFrenzyBg).style.animationDuration = `0.2s`
							...
							frenzyWar.timer -= 4 * MAIN_FPS
							if (field.frenzyWar.garbageContributed == 0 || field2.frenzyWar.garbageContributed == 0) {
								frenzyWar.timer -= (3 * MAIN_FPS) + 30
							}
									...
									if (selectedSettings.Other.Particle >= 2)
										for (var t = 0; t < 15; t++)
											GTRISParticleManagement.addParticle(
												...
												getElemPos(field.mainAssets.playField, "y") + (getElemPos(field.mainAssets.playField, "height") / 2),
												SCREEN_WIDTH * Math.random(),
												SCREEN_HEIGHT + (totalTetrionSize * (Math.random() * 10)),
												60 + (Math.random() * 70),
												5,
												"fallField"
											)
						...
						case 1.5 * MAIN_FPS: {
							if (field.frenzyWar.health <= 0 || field2.frenzyWar.health <= 0) {
								...
								field.are.frenzyEnt = field2.are.frenzyEnt = 15
							}
						}
					}
					if (time >= 5 * MAIN_FPS && time < 13 * MAIN_FPS && time % 15 == 0 && SCREEN_WIDTH * 0.8 > SCREEN_HEIGHT) {
						GTRISParticleManagement.addParticle(
							0,
