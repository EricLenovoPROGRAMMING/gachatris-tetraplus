const preview2 = new function() {
  ...
  this.draw = function() {
    clear(_CTX[field2.mainAssets.next]);
    clear(_CTX[field2.mainAssets.queue])
    for (var i = 0; i < Math.min(field2.pieceSettings.PREV, this.grabBag.length); i++) {
      if (i == 0) {
        if (this.grabBag[i] === 0) {
          draw(
